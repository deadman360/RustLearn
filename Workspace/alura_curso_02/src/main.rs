...
fn matriz() {
    let matriz: [[f32; 3]; 2] = [[0.1, 0.2, 0.3], [1.2, 1.3, 1.4]];
...
fn opcional() {
    ...
    if let Some(valor) = opcao {
        // if let é um eslito de match statment que podemos passar um padrao, nesse caso Some(valor) e um valor para match
        println!("{}", valor);
    }
}
fn opcao(init: &'static str) -> Option<&'static str> {
    //Option é um estilo de monada mas não é uma propriamente dita mas é um tipo em rust completamente similar a uma monada
    match init {
...
fn vectors() {
    ...
    println!(
        "Valor 50 = {}",
        match vetor_com_valores.get(50) {
            // nesse caso esse metodo .get retorna um option em vez de retornar diretamente um valor
...
impl Cliente {
    // impl Cliente diz que todas funções dentro desse mesmo bloco são métodos
    fn aniversario(&mut self, desconto: i32) {
        println!(
            "Ola CLiente {}, Voce recebeu um desconto de {}% na sua proxima compra, Feliz {} anos!",
            self.id,
            desconto,
            self.pessoa.idade + 1
        );
...
fn structs() {
    ...
    let mut eu_cliente: Cliente = Cliente {
        pessoa: eu,
        id: 3452,
    };
    println!(
        "Nome: {} {}, idade: {},ID: {} ",
        eu_cliente.pessoa.nome, eu_cliente.pessoa.sobrenome, eu_cliente.pessoa.idade, eu_cliente.id
    );
